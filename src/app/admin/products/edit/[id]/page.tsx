      ...
      if (
        ...
        message.error(
          `Không thể giảm tồn kho xuống dưới ${
            apiError.response.data.minimumAllowedPhysicalStock ?? 0
          } vì sẽ làm số lượng tồn kho nhỏ hơn mức tối thiểu hệ thống cho phép.`,
        );
        return;
      }
          ...
          <div>
            ...
            <div className="mt-4">
              ...
              <Text className="max-w-3xl text-base leading-7 text-gray-500">
                Tối ưu thông tin chung, kiểm soát tồn kho và quản lý biến thể theo màu sắc – kích thước trong cùng một màn hình.
              </Text>
            </div>
                        ...
                        <Col xs={24}>
                          <Form.Item label="Ảnh đại diện" name="imageUrl">
                            <div className="flex gap-2">
                              <Input placeholder="Nhập URL ảnh đại diện hoặc upload lên Cloudinary" />
                              <CloudinaryUploadButton
                                buttonText="Cloudinary"
                                onUploaded={(assets) => {
                                  const first = assets[0];
                                  if (first?.secureUrl) {
                                    form.setFieldValue("imageUrl", first.secureUrl);
                                  }
                                }}
                              />
                            </div>
            ...
            <div className="flex gap-2">
              ...
              <CloudinaryUploadButton
                ...
                onUploaded={(assets) => {
                  const first = assets[0];
                  if (first?.secureUrl) {
                    modalForm.setFieldValue("imageUrl", first.secureUrl);
                  }
                }}
              />
            </div>
                  ...
                  {[0, 1, 2, 3].map((index) => (
                    <div key={index} className="flex gap-2">
                      ...
                      <CloudinaryUploadButton
                        ...
                        onUploaded={(assets) => {
                          const first = assets[0];
                          if (!first?.secureUrl) return;
                          const current = (modalForm.getFieldValue("galleryImages") as string[] | undefined) || ["", "", "", ""];
                          current[index] = first.secureUrl;
                          modalForm.setFieldValue("galleryImages", current);
                        }}
                      />
                    </div>
                  ))}
              ...
              <div key={index} className="flex gap-2">
                ...
                <CloudinaryUploadButton
                  ...
                  onUploaded={(assets) => {
                    const first = assets[0];
                    if (first?.secureUrl) {
                      updateGenGallery(index, first.secureUrl);
                    }
                  }}
                />
              </div>
